...
const LayoutReading = ({ children }) => {
  ...
  const onClickFavoriteIcon = () => {
    try {
      post("/toggle-favorite", { userId, readingId, type })
        .then((response) => {
  ...
  const handleSendEmail = async () => {
    ...
    const url = "/send-reading-email";
    const readingUrl = `${hostingDomain}reading/client-detail/${readingId}`;

    try {
      dispatch(setIsLoading({ isLoading: true }));
      const result = await post(url, { email, readingUrl });
